...
async function renderNotesList() {
  ...
  notes.forEach(n => {
    ...
    `;
    el.querySelector('.note-item-text').addEventListener('click', () => {
      openNote(n.id);
      closeSidebar();
    });
    el.querySelector('.note-delete-btn').addEventListener('click', async (e) => {
      e.stopPropagation();
      await handleDeleteNote(n.id, 'note');
    });
  ...
  entries.forEach(e => {
    ...
    el.innerHTML = `
      <div class="note-item-row">
        ...
        <button class="note-delete-btn" title="Delete entry">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
            <polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6M14 11v6"/><path d="M9 6V4a1 1 0 0 1 1-1h4a1 1 0 0 1 1 1v2"/>
          </svg>
        </button>
    ...
    el.querySelector('.note-delete-btn').addEventListener('click', async (ev) => {
      ev.stopPropagation();
      await handleDeleteNote(e.id, 'journal');
    });
...
async function handleDeleteNote(id, type) {
  const confirmed = confirm('Delete this ' + (type === 'journal' ? 'journal entry' : 'note') + '? This cannot be undone.');
  if (!confirmed) return;
  ...
  // If it was open, go back to empty state
